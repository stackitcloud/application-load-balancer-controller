...
var _ = Describe("Metrics", func() {
	Describe("getSDKOperationName", func() {
		var (
			...
			host               string
			component          = "test"
			loadbalancerClient *alb.APIClient
		)
		...
		BeforeEach(func() {
			...
			loadbalancerClient, err = alb.NewAPIClient(
		...
		It("should return DeleteLoadBalancer as operation", func() {
			_, err := loadbalancerClient.DefaultAPI.DeleteLoadBalancer(context.TODO(), uuid.New().String(), "", uuid.New().String()).Execute()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should return DeleteCredentials as operation", func() {
			_, err := loadbalancerClient.DefaultAPI.DeleteCredentials(context.TODO(), uuid.New().String(), "", uuid.New().String()).Execute()
			Expect(err).NotTo(HaveOccurred())
			...
			labels := prometheus.Labels{
				...
				componentLabel: component,
				methodLabel:    "DELETE",
				operationLabel: "DeleteCredentials",
				codeLabel:      "200",
			}
